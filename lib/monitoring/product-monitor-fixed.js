    ...
    async scrapeCurrentProducts() {
        try {
            ...
            $('.product-grid-list-item').each((index, element) => {
                ...
                // FIXED: Check availability - look for "add to cart" button or availability indicators
                let availability = 'not available'; // Default to not available
                const itemText = $item.text().toLowerCase();
                const itemHtml = $item.html().toLowerCase();
                
                // Check for positive availability indicators
                if (itemHtml.includes('add to cart') || 
                    itemHtml.includes('add-to-cart') ||
                    itemHtml.includes('addtocart') ||
                    itemHtml.includes('add to bag') ||
                    itemHtml.includes('add-to-bag') ||
                    itemHtml.includes('addtobag') ||
                    $item.find('[class*="add"][class*="cart"], [class*="add"][class*="bag"], button[class*="add"]').length > 0) {
                    availability = 'available';
                } 
                // Check for negative availability indicators
                else if (itemText.includes('out of stock') || 
                         itemText.includes('sold out') ||
                         itemText.includes('unavailable')) {
                    availability = 'not available';
                }
                // If we previously detected it as "available" based on the word "available" in text
                else if (itemText.includes('available') && !itemText.includes('unavailable')) {
                    availability = 'available';
                }
        ...
        if (this.alertSettings.availabilityChanges && changes.availabilityChanges.length > 0) {
            for (const change of changes.availabilityChanges) {
                if (this.shouldAlert(change.product)) {
                    ...
                    else if (change.newAvailability === 'not available') emoji = '❌';
